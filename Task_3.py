...
def bytessub(state):
    ...
    for i in range(len(state)):
        for j in range(len(state[i])):
            state[i][j] = hex(sbox[int(state[i][j][0], 16)][int(state[i][j][1], 16)])[2:]
    return state


def shiftrow(state):
    shift_table = {0: {4: 0, 6: 0, 8: 0}, 1: {4: 1, 6: 1, 8: 1}, 2: {4: 2, 6: 2, 8: 3}, 3: {4: 3, 6: 3, 8: 4}}
    column_num = len(state[0])
    for i in range(len(state)):
        k = []
        for j in range(shift_table[i][column_num], shift_table[i][column_num] + column_num):
            k.append(state[i][j % column_num])
        state[i] = k
    return state
